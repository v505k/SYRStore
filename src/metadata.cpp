...
static string XmlEscapeText(const string& s)
{
	string r;
	r.reserve(s.size() + 16);
...
static bool WriteOTAInstallManifestPlist(const string& strPlistPath, const jvalue& jvMeta)
{
	string strFileName = jvMeta["FileName"];
	string strIconName = jvMeta["IconName"];
	string strBundle = jvMeta["AppBundleIdentifier"];
	string strVersion = jvMeta["AppVersion"];
	string strTitle = jvMeta["AppName"];

	string strXml;
	strXml.reserve(2048);
	strXml += "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
	strXml += "<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">\n";
	strXml += "<plist version=\"1.0\">\n";
	strXml += "<dict>\n";
	strXml += "<key>items</key>\n";
	strXml += "<array>\n";
	strXml += "<dict>\n";
	strXml += "<key>assets</key>\n";
	strXml += "<array>\n";
	strXml += "<dict>\n";
	strXml += "<key>kind</key>\n";
	strXml += "<string>software-package</string>\n";
	strXml += "<key>url</key>\n";
	strXml += "<string>" + XmlEscapeText(strFileName) + "</string>\n";
	strXml += "</dict>\n";
	strXml += "<dict>\n";
	strXml += "<key>kind</key>\n";
	strXml += "<string>display-image</string>\n";
	strXml += "<key>url</key>\n";
	strXml += "<string>" + XmlEscapeText(strIconName) + "</string>\n";
	strXml += "</dict>\n";
	strXml += "<dict>\n";
	strXml += "<key>kind</key>\n";
	strXml += "<string>full-size-image</string>\n";
	strXml += "<key>url</key>\n";
	strXml += "<string>" + XmlEscapeText(strIconName) + "</string>\n";
	strXml += "</dict>\n";
	strXml += "</array>\n";
	strXml += "<key>metadata</key>\n";
	strXml += "<dict>\n";
	strXml += "<key>bundle-identifier</key>\n";
	strXml += "<string>" + XmlEscapeText(strBundle) + "</string>\n";
	strXml += "<key>bundle-version</key>\n";
	strXml += "<string>" + XmlEscapeText(strVersion) + "</string>\n";
	strXml += "<key>kind</key>\n";
	strXml += "<string>software</string>\n";
	strXml += "<key>title</key>\n";
	strXml += "<string>" + XmlEscapeText(strTitle) + "</string>\n";
	strXml += "</dict>\n";
	strXml += "</dict>\n";
	strXml += "</array>\n";
	strXml += "</dict>\n";
	...
	if (!ZFile::WriteFile(strPlistPath.c_str(), strXml)) {
		return ZLog::ErrorV(">>> GetMetadata: Can't write %s\n", strPlistPath.c_str());
	}
	ZLog::PrintV(">>> OTA manifest:\t%s\n", strPlistPath.c_str());
	return true;
}
...
bool GetMetadata(const string& strAppFolder, const string& strOutputDir, const string& strIpaFile, bool bWriteJson)
{
	...
	string strPlistPath = strOutputDir + "/manifest.plist";
	if (!WriteOTAInstallManifestPlist(strPlistPath, jvMeta)) {
		return false;
	}
