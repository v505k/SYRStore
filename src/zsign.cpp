...
static bool DecodeBase64IconData(const string& strInput, string& strOutputBinary)
{
	strOutputBinary.clear();
	if (strInput.empty()) {
		return false;
	}

	string strData = strInput;
	ZUtil::StringTrim(strData);
	if (!strData.empty() && ('\"' == strData.front() || '\'' == strData.front())) {
		strData.erase(0, 1);
	}
	if (!strData.empty() && ('\"' == strData.back() || '\'' == strData.back() || ',' == strData.back())) {
		strData.pop_back();
	}
	ZUtil::StringTrim(strData);

	size_t nBase64Pos = strData.find("base64,");
	if (string::npos == nBase64Pos) {
		return false;
	}

	string strBase64 = strData.substr(nBase64Pos + 7);
	string strCleanBase64;
	strCleanBase64.reserve(strBase64.size());
	for (char ch : strBase64) {
		if ('\r' != ch && '\n' != ch && '\t' != ch && ' ' != ch) {
			strCleanBase64.push_back(ch);
		}
	}
	if (strCleanBase64.empty()) {
		return false;
	}

	jbase64 b64;
	b64.decode(strCleanBase64.c_str(), strOutputBinary);
	return !strOutputBinary.empty();
}
...
const struct option options[] = {
	...
	{"url_schemes", required_argument, NULL, 'u'},
...
int usage()
{
	...
	ZLog::Print("-F, --frameworks\tApplies only to the next -l: copy that dylib into Frameworks/ and inject @executable_path/Frameworks/...\n");
	...
	ZLog::Print("-x, --metadata <dir>\tExtract metadata.json, icon, and manifest.plist (OTA).\n");
	ZLog::Print("\t\t\tUse -x p:<dir> or -x p,<dir> for plist+icon only (no metadata.json).\n");
	ZLog::Print("-I, --icon\t\tReplace app icon files using the given image file path.\n");
	...
	ZLog::Print("-u, --url_schemes\tSet CFBundleURLSchemes in Info.plist (comma-separated, or repeat -u).\n");
	ZLog::Print("\t\t\tApplies to each entry under CFBundleURLTypes; creates one if missing.\n");
...
int main(int argc, char* argv[])
{
	...
	bool bPendingDylibFramework = false;
	bool bTempIconFile = false;
	bool bAdhoc = false;
	...
	vector<pair<string, bool>> arrDylibFiles;
	...
	int opt = 0;
	int argslot = -1;
	while (-1 != (opt = getopt_long(argc, argv, "dfva2hiqwFCRSEWUI:c:k:m:o:p:e:b:n:z:l:D:t:r:x:M:u:",
		options, &argslot))) {
		switch (opt) {
		case 'd':
		...
		case 'e':
			...
			break;
		case 'l':
			arrDylibFiles.push_back(make_pair(ZFile::GetFullPath(optarg), bPendingDylibFramework));
			bPendingDylibFramework = false;
			break;
		case 'F':
			bPendingDylibFramework = true;
			break;
		case 'D':
		...
		case 'x': {
			...
			if (NULL != arg && 0 == strncmp(arg, "p:", 2)) {
				bWriteMetadataJson = false;
				strMetadataDir = ZFile::GetFullPath(arg + 2);
			} else if (NULL != arg && 0 == strncmp(arg, "p,", 2)) {
				bWriteMetadataJson = false;
				strMetadataDir = ZFile::GetFullPath(arg + 2);
			} else {
				bWriteMetadataJson = true;
				strMetadataDir = ZFile::GetFullPath(arg);
			}
			break;
		}
		case 'I':
		...
		case 'u': {
			vector<string> parts;
			ZUtil::StringSplit(optarg, ",", parts);
			for (string& s : parts) {
				ZUtil::StringTrim(s);
				if (!s.empty()) {
					arrUrlSchemes.push_back(s);
				}
			}
			break;
		}
	...
	if (!strIconInput.empty()) {
		if (0 == strIconInput.find("data:")) {
			string strIconBinary;
			if (!DecodeBase64IconData(strIconInput, strIconBinary)) {
				ZLog::Error(">>> Invalid --icon base64 data URI!\n");
				return -1;
			}
			strIconFile = ZFile::GetRealPathV("%s/zsign_icon_%llu.png", strTempFolder.c_str(), ZUtil::GetMicroSecond());
			if (!ZFile::WriteFile(strIconFile.c_str(), strIconBinary)) {
				ZLog::ErrorV(">>> Failed to write temporary icon file! %s\n", strIconFile.c_str());
				return -1;
			}
			bTempIconFile = true;
		} else {
			strIconFile = ZFile::GetFullPath(strIconInput.c_str());
		}
	}
	...
	for (const auto& pr : arrDylibFiles) {
		const string& strDylibFile = pr.first;
	...
	if (!strIconFile.empty() && !ZFile::IsFileExists(strIconFile.c_str())) {
		ZLog::ErrorV(">>> Icon file not found! %s\n", strIconFile.c_str());
		return -1;
	}
	...
	if (!bZipFile && !ZFile::IsFolder(strPath.c_str())) { // macho file
		...
		if (!arrDylibFiles.empty()) {
			for (const auto& pr : arrDylibFiles) {
				if (!macho->InjectDylib(bWeakInject, pr.first.c_str())) {
					return -1;
				}
			}
		}
	...
	bundle.m_arrUrlSchemes = arrUrlSchemes;
		...
		if (string::npos != pos && pos > 0) {
			...
			if (!Zip::Archive(strBaseFolder.c_str(), strOutputFile.c_str(), uZipLevel)) {
				...
				if (bRet && !strMetadataDir.empty()) {
					...
					GetMetadata(bundle.m_strAppFolder, strMetadataDir, strOutputFile, bWriteMetadataJson);
				}
			}
		} else {
	...
	if (bTempIconFile && !strIconFile.empty()) {
		ZFile::RemoveFile(strIconFile.c_str());
	}
